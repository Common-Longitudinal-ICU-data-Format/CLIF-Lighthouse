...
def show_adt_qc():
    ...
    if 'root_location' in st.session_state and 'filetype' in st.session_state:
        ...
        if os.path.exists(filepath):
            ...
            def apply_changes(data, convert_dtypes, apply_deduplication, fix_overlaps):
                if convert_dtypes:
                ...
                if fix_concurrent:
                    overlaps = check_time_overlap(data)
                    data = fix_overlaps(data, overlaps)
            ...
            # if st.session_state:
                ...
                with st.form(key='apply_adt_changes_form'):
                    ...
                    if fix_concurrent:
                        fix_concurrent = st.checkbox("Revise patient `out_dttm` to avoid concurrent admissions")
                    else:
                        fix_concurrent = False
                ...
                if submit_button:
                    with st.spinner("Applying changes..."):
                        time.sleep(10)
                        revised_data = apply_changes(data.copy(), convert_dtypes, apply_deduplication, fix_overlaps)
