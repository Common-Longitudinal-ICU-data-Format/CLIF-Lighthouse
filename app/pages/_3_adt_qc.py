...
def show_adt_qc():
    ...
    if 'duckdb_conn' in st.session_state:
        ...
        # Query the DuckDB database for the ADT table
        query = f"SELECT * FROM adt"
        try:
            data = conn.execute(query).fetchdf()
            logger.info(f"Data for {TABLE} table loaded from DuckDB.")
        except Exception as e:
            st.error(f"Failed to load {TABLE} data from DuckDB: {e}")
            logger.error(f"Failed to load {TABLE} data from DuckDB: {e}")
            return

        # If data is loaded successfully, proceed with QC
        progress_bar = st.progress(0, text="Quality check in progress. Please wait...")
        logger.info(f"Data for {TABLE} loaded successfully.")
        ...
        with st.expander("Expand to view", expanded=False):
            ...
            with st.spinner("Checking for Overlapping Admissions..."):
                ...
                overlaps = check_time_overlap(data, conn)
                if len(overlaps) > 0:
        ...
        st.error("DuckDB connection not found in session state. Please upload files to proceed.")
        logger.warning("DuckDB connection not provided in session state.")
